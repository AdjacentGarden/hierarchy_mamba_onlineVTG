...
class SwiGLUFFN(nn.Module):
    def __init__(self, dim: int,
        ...
        if hidden_dim is None:
            hidden_dim = int(2 * (4 * dim) / 3) # 2.66× trick

        self.w12 = nn.Linear(dim, 2 * hidden_dim, bias=False)  # [gate | up] for LLaMA cmp.
        self.w2  = nn.Linear(hidden_dim, dim, bias=False)
        self.dropout_p = dropout

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        gate, up = self.w12(x).chunk(2, dim=-1)     # gate first, up second  ← LLaMA order
...
class MaskedMHA(nn.Module):
    ...
    def _chunk(self, x, size):
        ...
        # print(f"t: {t}, size: {size}, stride: {self.stride}")
